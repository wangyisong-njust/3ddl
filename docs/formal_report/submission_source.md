...
### Metrology as a Cliff-Sensitive Output
...
![Conceptual cliff effect of thresholded metrology: a small normalized boundary shift can cross the decision threshold and flip the output defect state.](../figures/metrology_cliff_effect.png)

### Industrial Deployment Challenge
...
### Difference from Earlier Acceleration Studies
...
## Main Contributions of This Report
...
## Evaluation Philosophy and the Two-Level Deployment-Safety Gate
...
## 3D Segmentation and Medical-Imaging Style Networks
...
## YOLO-Style Detection for Slice-Level Screening
...
## Mixed Precision and Runtime Deployment
...
## Semiconductor X-ray Inspection and Metrology
...
## Why the Current Problem Is Non-Trivial
...
# Design of a Defect-Aware Acceleration Framework
...
![End-to-end artifact flow of the current `intelliscan` default pipeline.](../figures/pipeline_overview_demo.png)

### One Concrete Demo from Input to Output
...
![Representative end-to-end demo on `SN002`, showing one detected bump, the cropped region, the predicted segmentation overlay, the stage timing, and the corresponding metrology row.](../figures/sn002_pipeline_demo.png)
...
### Full-Case and Deployment-Aware Selection
...
## Engineering Design for Safe Pipeline Acceleration
...
![Old separate segmentation-to-metrology path versus the retained combined path. The redesign removes the redundant per-bbox prediction write/read cycle while leaving segmentation geometry unchanged.](../figures/combined_seg_metrology_flow.png)

### Report Reconstruction Without Per-BBox Raw Crops
...
### CPU Preparation Parallelism and Batch Prefetch
...
## Algorithmic Design and Causal Ablation
...
## Two-Level Deployment-Safety Gate
...
![Two-Level Deployment-Safety Gate used throughout this report.](../figures/two_level_evaluation_gate.png)
...
## Baseline Profiling of the Whole Pipeline
...
Table: Baseline and accelerated formal benchmark on SN009
...
![Formal `SN009` stage breakdown across the baseline, earlier exploratory `--inmemory` run, and TRT FP16.](../figures/sn009_formal_stage_breakdown.png)

## Safe Engineering Optimizations

### Combined Segmentation and Metrology

Table: Combined segmentation and metrology stage comparison
...
### Report and Artifact Cleanup

Table: Report-memory-reuse result after removing `mmt/img` dependency
...
Table: No-`mmt/pred` GLTF stage benchmark
...
Table: Quantized in-memory detection against the older file-based detection baseline
...
Table: Current safe default path after worker parallelism and batch prefetch
...
![Eight-sample whole-pipeline reduction of the retained engineering path relative to the older file-based detection baseline.](../figures/engineering_cumulative_runtime_reduction.png)

## Algorithmic and Runtime Findings

### TRT FP16 Is Faster but Not Yet Safe

Table: TensorRT FP16 speed gain and quality risk on SN009
...
### Adaptive Margin: Causal Ablation

Table: Factor-isolation ablation on adaptive margin
...
![Adaptive-margin ablation on `SN009`, showing that context shrinkage reproduces most of the observed drift.](../figures/adaptive_margin_ablation_sn009.png)

### Guarded Adaptive Margin

The guarded search recovered a partial speedup, but not a fully safe default.

Table: Best guarded adaptive candidate on SN009
...
Table: Larger direct padded ROI is not a safe or worthwhile default optimization
...
### Pruning Pareto Front

Table: Pruning ratio Pareto ranking
...
![Pruning-ratio Pareto front for the retained KD recipe.](../figures/pruning_ratio_pareto.png)

### Knowledge Distillation Findings

Small-scale experiments first showed that class-aware KD was promising. Full-case evaluation then refined the decision.

Table: Full-case recovery ranking for the retained `0.5` student
...
## Deployment of Compressed Students into the Whole Pipeline

### Direct Student Deployment

Table: Whole-pipeline deployment of compressed students against the teacher
...
![Deployment gap between full-case average Dice and whole-pipeline defect-flip behavior.](../figures/deployment_gap_fullcase_vs_pipeline.png)
...
Table: Deployment-aware checkpoint selection
...
### Whole-Pipeline Runtime Candidate Ranking
...
Table: Whole-pipeline ranking of retained runtime candidates and related baselines
...
![Whole-pipeline runtime-quality-energy ranking derived from the candidate table. Marker area is proportional to mean whole-pipeline energy.](../figures/runtime_candidate_ranking_latency_quality.png)

### Case Study: Good Full-Case Dice, Bad Pipeline Behavior
...
![Representative deployment-gap case on `SN002`, where the compressed student changes the predicted component layout and downstream defect state despite a visually similar crop.](../figures/case_study_teacher_vs_student_sn002.png)

## Synthesis of Effective and Ineffective Directions
...
# Publications from this Report {.unnumbered}
